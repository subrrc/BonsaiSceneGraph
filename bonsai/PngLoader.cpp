...
#if !defined(_WIN32)
#error "PngLoader.cpp currently supports Windows only (WIC)."
#endif
...
// Link hint for MSVC (still recommend doing it in CMake/Project settings)
...
namespace bonsai {
    ...
    {
        // We can call CoInitializeEx multiple times; S_FALSE means already initialized on this thread.
        HRESULT hr = CoInitializeEx(nullptr, COINIT_MULTITHREADED);
        return SUCCEEDED(hr) || hr == RPC_E_CHANGED_MODE;
    }

    static void flipImageVerticallyRGBA(std::vector<std::uint8_t>& pixels, UINT w, UINT h)
    {
    ...
    std::shared_ptr<Image> PngLoader::loadRGBA(const std::string& filename)
    {
        if (filename.empty())
            return nullptr;
        ...
        // IMPORTANT: Flip vertical so UV (0,0) bottom-left displays correctly in OpenGL
        flipImageVerticallyRGBA(pixels, w, h);

        auto img = std::make_shared<Image>((int)w, (int)h, 4);
        img->data() = std::move(pixels); // marks dirty -> uploads lazily on bind
        return img;
    }
